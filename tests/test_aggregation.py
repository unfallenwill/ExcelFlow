...
class AggregationTest(unittest.TestCase):
    ...
    def test_all_grouped_aggregations_and_null_group(self):
        ...
        result = self.engine._aggregate(
            self.frame, ExtractionSpec(groups=self.groups, aggregations=rules), "summary"
        )
        self.assertEqual(
    ...
    def test_grouped_value_aggregations_are_asserted(self):
        ...
        # group A last_label == "x" (not None): locks groupby.last() NA-skipping.
    ...
    def test_multi_key_grouping_combines_columns(self):
        ...
        rules = [
            self.rule("", "count_all", "rows", "integer", 1),
            self.rule("o.amount", "sum", "total", "decimal", 2),
        ]
        result = self.engine._aggregate(
            frame, ExtractionSpec(groups=groups, aggregations=rules), "summary"
        )
    ...
    def test_whole_table_first_last_skips_na_and_returns_na_when_empty(self):
        ...
        result = self.engine._aggregate(
            ...
            ExtractionSpec(
                aggregations=[
                    ...
                    self.rule("o.tag", "last", "last", "string", 2),
                ]
            ),
            "summary",
        )
        ...
        empty_result = self.engine._aggregate(
            empty,
            ExtractionSpec(
                aggregations=[
                    self.rule("o.tag", "first", "first", "string", 1),
    ...
    def test_every_whole_table_aggregation(self):
        ...
        result = self.engine._aggregate(
            self.frame, ExtractionSpec(aggregations=rules), "summary"
        ).iloc[0]
        self.assertEqual(
            result.to_dict(),
            {
    ...
    def test_template_and_repository_support_optional_aggregation_sheets(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(spec.aggregations, [])
            del workbook["分组字段"]
            del workbook["聚合规则"]
            legacy = Path(directory) / "legacy.xlsx"
    ...
    def test_validator_rejects_invalid_aggregation_contracts(self):
        ...
        invalid = ExtractionSpec(
            **base,
            fields=[
                {"任务ID": "summary", "源字段": "o.id", "目标字段": "id", "目标类型": "integer"}
            ],
            groups=self.groups,
            aggregations=[self.rule("o.amount", "count_all", "department", "bad", 0)],
        )
        ...
        duplicate_groups = ExtractionSpec(
            **base,
            groups=[*self.groups, {**self.groups[0], "目标字段": "department_2", "分组顺序": 2}],
            aggregations=[self.rule("", "count_all", "rows", "integer", 1)],
        )
        self.assertTrue(
            any(
                "分组源字段不能重复" in error
                for error in SpecValidator().validate(duplicate_groups).errors
            )
        )
    ...
    def test_scalar_expression_is_broadcast_and_converted(self):
        ...
        self.assertEqual(
            result["date_value"].dt.strftime("%Y-%m-%d").tolist(), ["2024-01-02", "2024-01-02"]
        )
    ...
    def test_excel_plan_to_grouped_result_end_to_end(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            workbook["关联关系"].append(
                ["summary", 1, "LEFT JOIN", "o.order_id", "i", "i.order_id", ""]
            )
            workbook["过滤条件"].append(["summary", 1, 1, "o.status", "=", "paid", "", ""])
            workbook["分组字段"].append(["summary", "o.customer", "customer", "string", 1, ""])
            workbook["聚合规则"].append(
                ["summary", "", "count_all", "line_count", "integer", "", 1, ""]
            )
            workbook["聚合规则"].append(
                ["summary", "o.order_id", "count_distinct", "order_count", "integer", "", 2, ""]
            )
            workbook["聚合规则"].append(
                ["summary", "i.amount", "sum", "total", "decimal", "", 3, ""]
            )
            workbook.save(plan)
            with pd.ExcelWriter(source) as writer:
                ...
                ).to_excel(writer, sheet_name="订单", index=False)
                pd.DataFrame({"order_id": [1, 1, 2, 3], "amount": [10, 20, 5, 99]}).to_excel(
            ...
            self.assertIn("分组字段: 1 个", preview)
            self.assertIn("聚合规则: 3 条", preview)
            result = self.engine.execute(spec, "summary", source)
            self.assertEqual(
                result.to_dict("records"),
                [{"customer": "A", "line_count": 3, "order_count": 2, "total": 35.0}],
            )
            ...
            self.assertEqual(
                pd.read_csv(output).to_dict("records"),
                [{"customer": "A", "line_count": 3, "order_count": 2, "total": 35.0}],
            )
