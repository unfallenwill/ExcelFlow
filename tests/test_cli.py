...
class CliTest(unittest.TestCase):
    def run_cli(self, *args, cwd=None):
        ...
        try:
            ...
            with patch.object(sys, "argv", ["excelflow", *args]), contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                try: returncode = main()
                except SystemExit as exc: returncode = int(exc.code or 0)
        finally:
    ...
    def test_version_long_short_console_and_module_entrypoints(self):
        ...
        console = subprocess.run([executable, "--version"], capture_output=True, text=True, check=False)
        self.assertEqual(console.returncode, 0, console.stderr)
        self.assertEqual(console.stdout.strip(), expected)
        for option in ("--help", "--version"):
            module = subprocess.run([sys.executable, "-m", "excelflow", option], capture_output=True, text=True, check=False)
    ...
    def test_preview_long_and_short_options(self):
        commands = (("--plan", str(PLAN), "--task", "lesson_01"), ("-p", str(PLAN), "-t", "lesson_01"))
    ...
    def test_run_long_options_to_csv(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = self.run_cli(
                "run", "--plan", str(PLAN), "--task", "lesson_01",
    ...
    def test_run_short_options_for_every_output_format(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for output_format in ("csv", "jsonl", "xlsx"):
                ...
                result = self.run_cli(
                    "run", "-p", str(PLAN), "-t", "lesson_01", "-s", str(SOURCE),
                    "-f", output_format, "-o", str(output),
                )
                self.assertEqual(result.returncode, 0, result.stderr)
                self.assertEqual(result.stderr, "")
                self.assertTrue(output.exists())
                if output_format == "csv": records = pd.read_csv(output).to_dict("records")
                elif output_format == "jsonl": records = [json.loads(x) for x in output.read_text().splitlines()]
                else: records = pd.read_excel(output).to_dict("records")
                self.assertEqual(records, EXPECTED_RECORDS)

    def test_old_positional_syntax_is_rejected_for_every_command(self):
        commands = (
            ("template", "plan.xlsx"), ("validate", "plan.xlsx"),
            ("preview", "plan.xlsx", "task"),
            ("run", "plan.xlsx", "task", "source.xlsx", "csv", "output.csv"),
        )
        for args in commands:
            with self.subTest(command=args[0]): self.assert_parse_error(self.run_cli(*args))

    def test_every_required_option_is_enforced(self):
        commands = [("validate",), ("preview", "--task", "lesson_01"), ("preview", "--plan", str(PLAN))]
        run_pairs = [
            ("--plan", str(PLAN)), ("--task", "lesson_01"), ("--source", str(SOURCE)),
            ("--format", "csv"), ("--output", "/tmp/result.csv"),
        ]
        for missing_index in range(len(run_pairs)):
            args = [value for index, pair in enumerate(run_pairs) if index != missing_index for value in pair]
            commands.append(("run", *args))
        for args in commands:
            with self.subTest(args=args): self.assert_parse_error(self.run_cli(*args))

    def test_invalid_unknown_and_abbreviated_options_are_rejected(self):
        commands = (
            ("unknown",), ("validate", "--pla", str(PLAN)),
            ("run", "-p", str(PLAN), "-t", "lesson_01", "-s", str(SOURCE), "-f", "xml", "-o", "out.xml"),
        )
        for args in commands:
            with self.subTest(args=args): self.assert_parse_error(self.run_cli(*args))
    ...
    def test_runtime_failures_use_exit_one_and_stderr(self):
        ...
        disabled = self.run_cli(
            "run", "-p", str(ROOT / "extraction_plan.xlsx"), "-t", "demo_orders",
            "-s", "/missing/source.xlsx", "-f", "csv", "-o", "/tmp/disabled.csv",
        )
        ...
        missing_source = self.run_cli(
            "run", "-p", str(PLAN), "-t", "lesson_01",
            "-s", "/missing/source.xlsx", "-f", "csv", "-o", "/tmp/missing.csv",
        )
