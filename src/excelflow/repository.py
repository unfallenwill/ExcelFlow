...
class ExcelSpecRepository:
    ...
    sheets = {"抽取计划", "数据对象", "关联关系", "字段映射", "过滤条件"}
    ...
    def load(self, path: Path) -> ExtractionSpec:
        ...
        missing = self.sheets - set(wb.sheetnames)
        if missing:
            raise ValueError(f"缺少工作表: {', '.join(sorted(missing))}")
        return ExtractionSpec(
            plans=self._records(wb["抽取计划"]), objects=self._records(wb["数据对象"]),
            joins=self._records(wb["关联关系"]), fields=self._records(wb["字段映射"]),
