...
class SpecValidator:
    ...
    aggregate_functions = {"count", "count_all", "count_distinct", "sum", "avg", "min", "max", "first", "last", "concat_agg"}
    ...
    def validate(self, spec: ExtractionSpec) -> ValidationResult:
        ...
        for collection, label in ((spec.fields, "字段映射"), (spec.filters, "过滤条件")):
            for row, item in enumerate(collection, 2):
                ...
                if label == "字段映射":
                    ...
                    if str(item.get("目标类型") or "").strip().lower() not in self.target_types: errors.append(f"字段映射第{row}行: 目标类型必须从下拉框选择")
                if label == "过滤条件":
        ...
        for collection, label in ((spec.groups, "分组字段"), (spec.aggregations, "聚合规则")):
            for row, item in enumerate(collection, 2):
                if str(item.get("任务ID") or "").strip() not in aliases: errors.append(f"{label}第{row}行: 任务ID不存在")

        for task in task_ids:
            groups, aggregations = spec.for_task(spec.groups, task), spec.for_task(spec.aggregations, task)
            fields = spec.for_task(spec.fields, task)
            if groups and not aggregations: errors.append(f"任务 {task}: 配置分组字段时必须至少配置一条聚合规则")
            if aggregations and fields: errors.append(f"任务 {task}: 聚合任务不能同时配置字段映射")
            targets: list[str] = []
            group_sources: list[str] = []
            for label, rows, order_field in (("分组字段", groups, "分组顺序"), ("聚合规则", aggregations, "聚合顺序")):
                orders: list[int] = []
                for row, item in enumerate(rows, 2):
                    source, target = str(item.get("源字段") or "").strip(), str(item.get("目标字段") or "").strip()
                    function = str(item.get("聚合函数") or "").strip().lower()
                    if label == "聚合规则" and function not in self.aggregate_functions: errors.append(f"聚合规则第{row}行: 不支持的聚合函数")
                    if label == "聚合规则" and function == "count_all" and source: errors.append(f"聚合规则第{row}行: count_all 不应填写源字段")
                    if (label == "分组字段" or function != "count_all") and (not QUALIFIED_FIELD.fullmatch(source) or source.split(".")[0] not in aliases[task]): errors.append(f"{label}第{row}行: 源字段引用无效")
                    if label == "分组字段": group_sources.append(source)
                    if not SAFE_FIELD.fullmatch(target): errors.append(f"{label}第{row}行: 目标字段不合法")
                    targets.append(target)
                    if str(item.get("目标类型") or "").strip().lower() not in self.target_types: errors.append(f"{label}第{row}行: 目标类型必须从下拉框选择")
                    try:
                        order = int(item.get(order_field));
                        if order < 1: raise ValueError
                        orders.append(order)
                    except (TypeError, ValueError): errors.append(f"{label}第{row}行: {order_field}必须是正整数")
                if len(orders) != len(set(orders)): errors.append(f"任务 {task}: {order_field}不能重复")
            if len(targets) != len(set(targets)): errors.append(f"任务 {task}: 分组或聚合目标字段存在重复")
            if len(group_sources) != len(set(group_sources)): errors.append(f"任务 {task}: 分组源字段不能重复")
