...
class SafeExpressionEvaluator:
    ...
    _date_units = {"day": "D", "days": "D", "hour": "h", "hours": "h", "minute": "m", "minutes": "m"}
    ...
    @staticmethod
    def _strings(value):
        if isinstance(value, pd.Series): return value.astype("string")
    ...
    def _node(self, node: ast.AST, frame: pd.DataFrame):
        ...
        if isinstance(node, ast.Compare):
            left = self._node(node.left, frame); masks = []
            operations = {ast.Eq: lambda a, b: a == b, ast.NotEq: lambda a, b: a != b,
                          ast.Gt: lambda a, b: a > b, ast.GtE: lambda a, b: a >= b,
                          ast.Lt: lambda a, b: a < b, ast.LtE: lambda a, b: a <= b}
            for operator, comparator in zip(node.ops, node.comparators):
                right = self._node(comparator, frame)
                if type(operator) not in operations: break
                masks.append(operations[type(operator)](left, right)); left = right
            else:
                return reduce(lambda a, b: a & b, masks)
        if isinstance(node, ast.BoolOp) and isinstance(node.op, (ast.And, ast.Or)):
            values = [self._node(value, frame) for value in node.values]
            operation = (lambda a, b: a & b) if isinstance(node.op, ast.And) else (lambda a, b: a | b)
    ...
    def _call(self, name: str, args: list):
        if name == "coalesce" and len(args) == 2:
            return args[0].fillna(args[1]) if self._series(args[0]) else args[0] if pd.notna(args[0]) else args[1]
        ...
        if name == "clip" and len(args) == 3 and self._series(args[0]): return args[0].clip(args[1], args[2])
        if name in {"upper", "lower", "trim", "length"} and len(args) == 1:
            value = self._strings(args[0])
            if self._series(value):
                return {"upper": value.str.upper, "lower": value.str.lower,
                        "trim": value.str.strip, "length": value.str.len}[name]()
            return {"upper": value.upper, "lower": value.lower,
                    "trim": value.strip, "length": value.__len__}[name]()
        if name == "replace" and len(args) == 3:
            value = self._strings(args[0]); old, new = str(args[1]), str(args[2])
            return value.str.replace(old, new, regex=False) if self._series(value) else value.replace(old, new)
        if name == "substring" and len(args) in {2, 3}:
            value, start = self._strings(args[0]), int(args[1]); stop = start + int(args[2]) if len(args) == 3 else None
        ...
        if name in {"contains", "startswith", "endswith"} and len(args) == 2:
            ...
            if self._series(value): return getattr(value.str, name)(pattern, na=False, regex=False) if name == "contains" else getattr(value.str, name)(pattern, na=False)
        ...
        if name == "concat_ws" and len(args) >= 2:
            ...
            if any(self._series(value) for value in values):
                ...
                series = [value if self._series(value) else pd.Series(value, index=index, dtype="string") for value in values]
        ...
        if name == "to_number" and len(args) == 1: return pd.to_numeric(args[0], errors="raise")
        if name == "to_string" and len(args) == 1: return self._strings(args[0])
        if name == "to_date" and len(args) == 1: return self._dates(args[0])
        if name == "dateformat" and len(args) == 2:
            value = self._dates(args[0]); return value.dt.strftime(str(args[1])) if self._series(value) else value.strftime(str(args[1]))
        if name in {"year", "month", "day"} and len(args) == 1:
            value = self._dates(args[0]); return getattr(value.dt, name) if self._series(value) else getattr(value, name)
        if name == "date_add" and len(args) == 3 and str(args[2]).lower() in self._date_units:
            return self._dates(args[0]) + pd.to_timedelta(args[1], unit=self._date_units[str(args[2]).lower()])
        if name == "date_diff" and len(args) == 3 and str(args[2]).lower() in self._date_units:
            delta = self._dates(args[0]) - self._dates(args[1]); unit = self._date_units[str(args[2]).lower()]
            return delta / pd.to_timedelta(1, unit=unit)
        if name == "is_null" and len(args) == 1: return pd.isna(args[0])
        ...
        if name in {"min_value", "max_value"} and len(args) >= 1:
            series = [value if self._series(value) else pd.Series(value, index=next(x.index for x in args if self._series(x))) for value in args] if any(self._series(x) for x in args) else args
            if any(self._series(x) for x in args): return pd.concat(series, axis=1).min(axis=1) if name == "min_value" else pd.concat(series, axis=1).max(axis=1)
