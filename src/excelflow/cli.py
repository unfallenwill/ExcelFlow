...
def main() -> int:
    ...
    run = sub.add_parser("run"); run.add_argument("path"); run.add_argument("task_id"); run.add_argument("source_excel")
    ...
    try:
        if args.command == "template": create_template(Path(args.path)); print(f"已生成: {args.path}")
            ...
            count, output = service.run(Path(args.path), args.task_id, Path(args.source_excel)); print(f"抽取完成: {count} 行 -> {output}")
