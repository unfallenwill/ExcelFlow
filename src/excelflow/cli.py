...
def main() -> int:
    parser = argparse.ArgumentParser(prog="excelflow", description="Excel 驱动的 Pandas 数据抽取工具", allow_abbrev=False)
    parser.add_argument("-V", "--version", action="version", version=f"excelflow {package_version()}")
    ...
    run.add_argument("-f", "--format", required=True, choices=["csv", "jsonl", "xlsx"], help="输出格式")
    run.add_argument("-o", "--output", required=True, help="输出路径")
    args, service = parser.parse_args(), ExtractionService()
    try:
        if args.command == "template": create_template(Path(args.output)); print(f"已生成: {args.output}")
        elif args.command == "validate":
            result = service.validate(Path(args.plan))
            for error in result.errors: print(f"错误: {error}", file=sys.stderr)
            if result.ok: print("校验通过")
            return 0 if result.ok else 1
        elif args.command == "preview": print(service.preview(Path(args.plan), args.task))
        else:
            count, output = service.run(Path(args.plan), args.task, Path(args.source), args.format, Path(args.output)); print(f"抽取完成: {count} 行 -> {output}")
