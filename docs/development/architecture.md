...
表达式能力应通过显式 AST 节点和函数白名单扩展，不能改用 `eval()`。新增配置字段时应同时更新模板、Schema 常量、Repository/Validator、教程与测试。
