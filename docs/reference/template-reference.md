...
| 字段 | 必填 | 允许值或格式 | 说明 |
|---|---:|---|---|
| 任务ID | 是 | 非空文本，本表唯一；不得含路径分隔符、Windows 非法文件名字符或控制字符，也不能为 `.`、`..` | 单任务模式下即 `--task` 参数值；多任务模式（省略 `--task`）下作为输出文件名 `<任务ID>.<格式>`。其他工作表通过它引用任务。 |
...
全空组行为：`sum`、`avg`、`min`、`max`、`first`、`last` 返回**空值**；`concat_agg` 返回**空字符串**；`count` 与 `count_distinct` 返回 `0`；`count_all` 返回行数。`sum` 与 `avg` 要求源字段可转为数值，否则任务失败。
