...
| 哪些订单要保留 | 过滤条件 |
| 最终输出哪些列、怎样计算 | 字段映射 |
| 是否需要按组计数、求和或汇总 | 分组字段、聚合规则 |

先写目标能避免“为了填满模板而填写”。没有关联时，关联关系可以为空；没有过滤时，过滤条件也可以为空。普通明细任务使用字段映射；汇总任务使用分组字段和聚合规则，两种模式不要混用。
